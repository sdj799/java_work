...
/*
		 ...
		 - 매서드는 반복되는 코드 블록에 이름을 붙여놓은 것입니다.
		 ...
		 - 매서드의 호출은 곧 매서드를 사용하는 것을 의미하며,
		 1.메서드가 필요로 하는 데이터(매개값)를 호출부에서 전달하면
		 2.그 값을 받아서 매서드 내부로 전달하고, 로직이 실행됩니다.
		 3.실행 결과값을 메서드 호출한 곳으로 다시 반환합니다.
		 4.반환된 값은 변수에 담아서 저장하거나, 출력하는 등 다양하게 활용합니다.
 */

public class MethodExample1 {
	
	//매서드의 선언! (static은 뒤에서 알려드릴게요. 지금은 없다고 생각하세요.)
	static int calcTotal(int x)//매개변수
	{
		int total = 0;
		for(int i=1; i<=x; i++) {
			total += i;
