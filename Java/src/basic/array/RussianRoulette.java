...
public class RussianRoulette {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i<playerNum; i++) {
			System.out.printf("%d번 플레이어 이름을 등록하세요: ", i+1);
			String name = sc.next();
			player[i] = name;
		}
		...
		while(true) {
			...
			if(bulletPos[realBulletPos]) {
				...
				for(int i =startIdx; i<playerNum; i++) {
					player[i]=player[i+1];
				}
				...
				player = temp; temp = null;
				
				System.out.println("생존 인원: " + Arrays.toString(player));
				
				if(playerNum == 1) {
					System.out.println("최종 생존자: " + player[0]);
					System.out.println("게임을 종료합니다.");
					break;
				} else if(bullet == 0) {
					System.out.println("총알이 다 떨어졌습니다.");
					System.out.println("최종생존자: " + Arrays.toString(player));
					System.out.println("게임을 종료합니다.");
					break;
				} else {
