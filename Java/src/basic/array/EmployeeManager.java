					...
					if(inputNum.equals(userNums[i])) {
						...
						outer:		while(true) {
							...
							case 1:
								...
								ages[i] = sc.nextInt();
								System.out.println("나이가 "+ages[i]+"세로 정상 변경되었습니다.");
								break;
							case 2:
								System.out.println("변경하실 부서를 입력하세요: ");
								departments[i] = sc.next();
								System.out.println("부서가 "+departments[i]+"로 변경되었습니다.");
							case 3:
					...
					if(modiNum.equals(userNums[i])) {
						...
						case "Y": case "y": case "ㅛ":
							...
							for(int n=i; n<count-1; n++) {
								userNums[n] = userNums[n+1];
								names[n] = names[n+1];
								ages[n] = ages[n+1];
								departments[n] = departments[n+1];
							}
							count--;
